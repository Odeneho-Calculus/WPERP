...
//            $caption = addslashes(trim($_POST['caption']));
           ...
           $description = str_replace( array( '\'', '<p>',
            ...
            if ($credit >= $mob) {
                ...
                $stmt = $conn->prepare("INSERT INTO `send_wp_msgs`(login_id,`campaign_unique_id`,`campaign_name`, `message`, `number_count`,`created_at`,`sort_date_wise`) VALUES (?,?,?,?,?,?,?)");
                $stmt->bind_param("isssiss", $login_id, $camp_unique_id, $caption, $message, $number_count,  $created_date, $sort_date);
                    if ($stmt->execute()) {
