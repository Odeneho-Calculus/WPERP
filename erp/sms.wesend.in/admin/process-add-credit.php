...
//				print_r($_FILES);
    ...
    $stmt2 = $conn2->prepare("Select credit from logins where logins.user_unique_id =? ");
    $stmt2->bind_param("s", $reseller_unique_id);
    $stmt2->execute();
    $stmt2->bind_result($old_credit);
    $stmt2->fetch();
    $conn2->close();
//    $old_credit;
//    $credit;
            ...
            $stmt->bind_param("dddsiddsssss", $no_of_sms, $per_sms_price, $old_credit, $tax_status, $tax_percentage, $tax_amount, $total_amount, $description, $unique_id, $reseller_unique_id, $txn_type, $created_date);
            if ($stmt->execute()) {
