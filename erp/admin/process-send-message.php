...
//                    echo $path;
                   ...
                   // $url = "https://api.bulkwhatsapp.net/wapp/api/send?mobile=$nums&msg=$msg&apikey=d09ee76524644aae99741a73226e6166$path";
                    $url = "https://api4ws.com/sendMessage.php?AUTH_KEY=917222994518$path&message=$msg&phone=$nums";
//                    echo $url;
//                    exit;
                   ...
                   // $result = curl_exec($ch); not calling api for instant delivery
                    ...
                    $stmt = $conn->prepare("INSERT INTO `send_wp_msgs`(login_id,`campaign_unique_id`,`campaign_name`, `message`, `number_count`, `image-1`, `image-2`, `image-3`, `image-4`, `upload_pdf`, `send_audio`, `send_video`, `dp_image`,`repybtn1`,`repybtn2`,`ctabtn1`,`ctabtn2`, status, `created_at`,`sort_date_wise`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->bind_param("isssisssssssssssssss", $login_id, $camp_unique_id, $caption, nl2br($_POST['description']), $number_count, $pimagename1, $pimagename2, $pimagename3, $pimagename4, $pdf_f, $audio_f, $video_f, $dp_image, $rplybtn1, $rplybtn2, $ctabtn1, $ctabtn2, $status, $created_date, $sort_date);
                    if ($stmt->execute()) {
                  ...
                  // End Section Of API
                    ...
                    $stmt = $conn->prepare("INSERT INTO `send_wp_msgs`(login_id,`campaign_unique_id`,`campaign_name`, `message`, `number_count`, `image-1`, `image-2`, `image-3`, `image-4`, `upload_pdf`, `send_audio`, `send_video`, `dp_image`,`repybtn1`,`repybtn2`,`ctabtn1`,`ctabtn2`, `created_at`,`sort_date_wise`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    $stmt->bind_param("isssissssssssssssss", $login_id, $camp_unique_id, $caption, nl2br($_POST['description']), $number_count, $pimagename1, $pimagename2, $pimagename3, $pimagename4, $pdf_f, $audio_f, $video_f, $dp_image, $rplybtn1, $rplybtn2, $ctabtn1, $ctabtn2, $created_date, $sort_date);
                    if ($stmt->execute()) {
                        $wp_insert_id = $stmt->insert_id;
